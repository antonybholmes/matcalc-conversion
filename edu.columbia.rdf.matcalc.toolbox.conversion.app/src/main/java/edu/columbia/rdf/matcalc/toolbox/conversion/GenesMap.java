...
public class GenesMap {
	...
	private Map<String, String> mIdToSymbolMap = 
			new HashMap<String, String>();

	private Map<String, String> mAltIdToSymbolMap = 
			new HashMap<String, String>();

	private Map<String, String> mOldIdToSymbolMap = 
			new HashMap<String, String>();
	...
	/**
	 * Add an unofficial name mapping. These are considered still valid
	 * and not retired.
	 * 
	 * @param symbol
	 * @param name
	...
	/**
	 * Convert symbol also checking that if the symbol is an old symbol
	 * that is converted to a new symbol, the new symbol does not have
	 * a homology since homology is used as the gold standard. Thus is
	 * there is a homology between species, the conversion must be by
	 * homology only and not simple name similarity mapping.
	 * 
	 * @param c
	 * @param split
	 * @param conversionMap
	...
	public void convert(Conversion c,
		...
		if (split) {
			...
			/*
				...
				for (String term : terms) {
					String s = mIdToSymbolMap.get(term);
	...
	/**
	 * For a given id, return any mappings of a given type. type will be
	 * of the form "entrez", "refseq", "ensembl-gene" etc depending on the
	...
	public static void loadEnsembl(final Path file, GenesMap genesMap) throws IOException {
		String line;
		List<String> tokens;
